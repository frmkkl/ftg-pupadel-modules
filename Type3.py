...
class TyperMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig("TYPE_CHAR", "▒", lambda m: self.strings("type_char_cfg_doc", m),
                                          ...
                                          "CHARACTERS_AT_A_TIME", 3, lambda m: self.strings("delay_text_cfg_doc", m))

    @loader.ratelimit
    async def type3cmd(self, message):
        """.type3 <message>"""
        a = utils.get_args_raw(message)
        a = analator(a, self.config["CHARACTERS_AT_A_TIME"])
